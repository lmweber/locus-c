---
title: "LC analyses: preprocessing summaries"
author: "Lukas Weber"
date: "`r Sys.Date()`"
output:
  html_document:
...
Computational analysis pipeline for LC project: preprocessing summaries.
...
for (i in seq_along(sample_names)) {
  ...
  hist(log10(colData(spe)$sum), breaks = 20, 
...
Marker genes for major cell types, and strategies for segmenting LC region.
...
# plot expression of neuron markers
for (i in seq_along(sample_names)) {
  
  spe <- spe_list[[i]]
  
  for (g in seq_along(gns)) {
    ...
    p <- ggplot(as.data.frame(cbind(colData(spe), spatialCoords(spe))), 
      ...
      labs(color = "counts") + 
      ggtitle(paste0("UMI counts: ", sample_names[i], ": ", gn)) + 
      theme_bw() + 
      theme(panel.grid = element_blank(), 
            axis.title = element_blank(), 
            axis.text = element_blank(), 
...
```{r, fig.width=3.75, fig.height=3.25}
# genes of interest
gns <- c("SNAP25", "GAD1", "TH", "SLC6A2", "MOBP", "PDGFRA", "C3", "GFAP")
...
thresh_umi <- c(3000, 1500, 1500, 1500, 1500)
...
for (i in seq_along(sample_names)) {
  ...
  # compare with marker expression
  gn <- "SNAP25"
  ix <- which(rowData(spe)$gene_name == gn)
  colData(spe)$sum_gn <- counts(spe)[ix, ]
  # note: remove duplicated columns introduced when adding columns to colData
...
# Spots expressing TH or SLC6A2

Calculate number of spots with high expression of either TH or SLC6A2, as an approximation for the proportion of spots in the LC region of interest.

This is more accurate than segmenting on total UMI counts, since total UMI counts also picks up other cell types.


## Plots and summary values
...
# calculate and plot thresholded regions
for (i in seq_along(sample_names)) {
  ...
  colData(spe)$sum_TH <- counts(spe)[ix_TH, ]
  colData(spe)$sum_SLC6A2 <- counts(spe)[ix_SLC6A2, ]
  colData(spe)$contains_NE_neuron <- (colData(spe)$sum_TH > thresh_TH) | (colData(spe)$sum_SLC6A2 > thresh_SLC6A2)
  # note: remove duplicated columns introduced when adding columns to colData
  colData(spe) <- colData(spe)[, !(duplicated(colnames(colData(spe))) | duplicated(colnames(colData(spe)), fromLast = TRUE))]
  ...
  # plot thresholded regions
  p <- ggplot(as.data.frame(cbind(colData(spe), spatialCoords(spe))), 
              aes(x = y, y = x, color = contains_NE_neuron)) + 
    geom_point(size = 0.5) + 
    coord_fixed() + 
    scale_y_reverse() + 
    scale_color_manual(values = c("gray90", "magenta")) + 
    labs(color = "NE_neurons") + 
    ggtitle(paste0("Identified spots: ", sample_names[i])) + 
    theme_bw() + 
    theme(panel.grid = element_blank(), 
          axis.title = element_blank(), 
          axis.text = element_blank(), 
  ...
  # summary values
  print(paste0("Sample name: ", sample_names[i]))
  
  print("Table of 'contains_NE_neuron':")
  print(table(colData(spe)$contains_NE_neuron))
  
  print(paste0("Mean: ", round(mean(colData(spe)$contains_NE_neuron), 3)))
  
  print(paste0("Number of spots over tissue: ", ncol(spe)))
}
```
