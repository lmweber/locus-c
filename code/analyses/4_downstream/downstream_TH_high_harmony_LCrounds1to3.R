#########################################################################
# LC project
# Downstream analyses on TH+ spots (high): with Harmony batch integration
...
dim(spe)  ## 666 spots across all samples
...
ggplot(df, aes(x = UMAP1, y = UMAP2, color = label)) + 
  geom_point(size = 0.3) + 
  ggtitle("UMAP: TH+ spots (high), Harmony, cluster labels") + 
  theme_bw() + 
  theme(panel.grid = element_blank(), 
        axis.text = element_blank(), 
        axis.ticks = element_blank())
ggsave(paste0(here("plots", "THpos_high_harmony", "THpos_high_harmony_clustering_labels"), ".pdf"), width = 4.5, height = 4)
ggsave(paste0(here("plots", "THpos_high_harmony", "THpos_high_harmony_clustering_labels"), ".png"), width = 4.5, height = 4)

# plot clustering: sample IDs
ggplot(df, aes(x = UMAP1, y = UMAP2, color = sample_id)) + 
  geom_point(size = 0.3) + 
  ggtitle("UMAP: TH+ spots (high), Harmony, sample IDs") + 
  theme_bw() + 
  theme(panel.grid = element_blank(), 
        axis.text = element_blank(), 
        axis.ticks = element_blank())
ggsave(paste0(here("plots", "THpos_high_harmony", "THpos_high_harmony_clustering_sampleIDs"), ".pdf"), width = 5.5, height = 4)
ggsave(paste0(here("plots", "THpos_high_harmony", "THpos_high_harmony_clustering_sampleIDs"), ".png"), width = 5.5, height = 4)
...
ggplot(df, aes(x = x, y = y, color = label)) + 
  ...
  coord_fixed() + 
  scale_y_reverse() + 
  ggtitle("TH+ spots (high), Harmony, cluster labels by sample") + 
  theme_bw() + 
  theme(panel.grid = element_blank(), 
