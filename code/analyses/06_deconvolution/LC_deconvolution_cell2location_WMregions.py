...
# remove NE neurons cluster ("Neuron.NE")
ix_remove <- colData(sce.lc)$cellType.collapsed %in% c("Neuron.NE")
table(ix_remove)
sce.lc <- sce.lc[, !ix_remove]
colData(sce.lc)$cellType.collapsed <- droplevels(colData(sce.lc)$cellType.collapsed)
table(colData(sce.lc)$cellType.collapsed)
...
adata_file = f"{ref_run_name}/sc_WMregions.h5ad"
...
# save anndata object with results
adata_file = f"{run_name}/sp_WMregions.h5ad"
...
fn <- here("processed_data", "SPE", "LC_cell2location_WMregions.rds")
saveRDS(spe, file = fn)
