...
# remove non-neuron cell types / clusters ("Astro", "Endo.Mural", "Micro.Macro", "Oligo", "OPC")
ix_remove <- colData(sce.lc)$cellType.collapsed %in% c("Astro", "Endo.Mural", "Micro.Macro", "Oligo", "OPC")
table(ix_remove)
sce.lc <- sce.lc[, !ix_remove]
colData(sce.lc)$cellType.collapsed <- droplevels(colData(sce.lc)$cellType.collapsed)
table(colData(sce.lc)$cellType.collapsed)
...
# save anndata object with results
adata_file = f"{run_name}/sp_neurons.h5ad"
...
fn <- here("processed_data", "SPE", "LC_cell2location_neurons.rds")
saveRDS(spe, file = fn)
