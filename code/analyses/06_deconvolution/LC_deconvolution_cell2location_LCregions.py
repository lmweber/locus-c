...
# remove 5-HT neurons cluster ("Neuron.5HT")
ix_remove <- colData(sce.lc)$cellType.collapsed %in% c("Neuron.5HT")
table(ix_remove)
sce.lc <- sce.lc[, !ix_remove]
colData(sce.lc)$cellType.collapsed <- droplevels(colData(sce.lc)$cellType.collapsed)
table(colData(sce.lc)$cellType.collapsed)
...
adata_file = f"{ref_run_name}/sc_LCregions.h5ad"
...
# save anndata object with results
adata_file = f"{run_name}/sp_LCregions.h5ad"
...
fn <- here("processed_data", "SPE", "LC_cell2location_LCregions.rds")
saveRDS(spe, file = fn)
