...
# pseudobulk spots in annotated spots (within LC regions) vs. not annotated
# spots (within LC regions)

# note: pseudobulk by sample_id

table(colData(spe)$sample_id, colData(spe)$annot_region)
table(colData(spe)$sample_id, colData(spe)$annot_spot)


# subset to keep only spots within LC regions
spe <- spe[, colData(spe)$annot_region]
dim(spe)


annot_spot_fctr <- factor(as.numeric(colData(spe)$annot_spot), 
                          labels = c("nonspot", "spot"))
table(annot_spot_fctr)

ids <- DataFrame(
  annot_spot_pseudo = annot_spot_fctr, 
  sample_id_pseudo = colData(spe)$sample_id
)

table(ids$sample_id_pseudo, ids$annot_spot_pseudo)
...
levs <- paste(
  colData(spe_pseudo)$annot_spot_pseudo, 
...
# e.g. threshold 40 UMI counts = 8 samples * 5 UMI counts per sample
thresh <- 40
...
# define model: annotated spots (within LC regions) vs. not annotated spots (within LC regions)
model_formula <- ~annot_spot_pseudo
...
# extract p-values
p_vals <- res$p.value[, "annot_spot_pseudospot"]
...
logFC <- res$coefficients[, "annot_spot_pseudospot"]
...
ggplot(df, aes(x = logFC, y = -log10(FDR), 
  ...
  geom_hline(yintercept = 2, lty = "dashed", color = "royalblue") + 
...
fn <- file.path(dir_plots, "pseudobulk_withinLCregions", "pseudobulkDE_withinLCregions_volcano")
...
# save heatmap
fn <- file.path(dir_plots, "pseudobulk_withinLCregions", "pseudobulkDE_withinLCregions_heatmap")
...
fn <- file.path(dir_plots, "pseudobulk_withinLCregions", "pseudobulkDE_withinLCregions_MAplot")
ggsave(paste0(fn, ".pdf"), width = 5, height = 4)
ggsave(paste0(fn, ".png"), width = 5, height = 4)
...
# save .csv file
fn <- file.path(dir_outputs, "pseudobulk_withinLCregions", 
                "LC_pseudobulkDE_withinLCregions_topGenes.csv")
