...
for (i in seq_along(sample_names)) {
  ...
  dim(W)
  head(W)
  # composition matrix (NMF cell types * genes)
  H <- coef(nmf_out)
  dim(H)
  H[, 1:6]
}
...
for (i in seq_along(sample_names)) {
  ...
  # composition matrix (transposed: genes * NMF cell types)
  H <- coef(nmf_out)
  df_H <- t(H)
...
Note: have we excluded highly specific overall neuron markers since we have only kept markers that can distinguish excitatory from inhibitory neurons? Could we generate an additional marker set with all neurons in a single category?
