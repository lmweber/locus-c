...
## Analysis pipeline within segmented region
...
Plot spatial distributions of expression for cluster marker genes identified in analysis pipeline above.
...
for (i in seq_along(sample_names)) {
  ...
  for (k in seq_along(top_genes)) {
    ...
    p <- ggplot(df, aes(x = y, y = x, color = logexpr)) + 
      ...
      coord_fixed() + 
      scale_y_reverse() + 
      scale_color_gradient(low = "gray90", high = "blue3") + 
